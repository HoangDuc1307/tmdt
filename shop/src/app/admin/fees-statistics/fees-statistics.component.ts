...
export class FeesStatisticsComponent implements OnInit, OnDestroy {
  ...
  private renderChart(): void {
    ...
    this.chart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          { label: 'Platform Fee', data: this.ts.platform_fee ?? [], backgroundColor: 'rgba(245, 158, 11, 0.35)', borderColor: '#f59e0b', borderWidth: 1 },
          { label: 'Revenue', data: this.ts.revenue ?? [], backgroundColor: 'rgba(37, 99, 235, 0.25)', borderColor: '#2563eb', borderWidth: 1 },
        ],
      },
      options: { 
        responsive: true, 
  ...
  saveReport(): void {
    if (!this.stats || !this.ts) { this.saveMessage = 'No data to save.'; return; }
    ...
    this.adminService.exportFeesReport(days).pipe(
      ...
      next: (blob: any) => {
        ...
        this.saveMessage = 'Fees report downloaded (Excel).';
      },
      error: () => {
        this.saveMessage = 'Export failed.';
      },
    });
  }
}
