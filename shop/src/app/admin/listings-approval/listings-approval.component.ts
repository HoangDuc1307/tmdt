...
export class ListingsApprovalComponent implements OnInit {
  ...
  load(): void {
    ...
    this.adminService.getListings(
      ...
      error: () => {
        this.error.set('Something went wrong, could not fetch listings.');
        this.loading.set(false);
      },
    });
  }
  ...
  approve(id: number): void {
    ...
    this.adminService.approveListing(id).subscribe({
      ...
      error: () => this.error.set('Approval failed. Please check Admin permissions or CSRF.'),
    });
  }

  // Từ chối bài đăng - bắt buộc phải nhập lý do để báo về cho Seller
  reject(id: number): void {
    const reason = window.prompt('Enter rejection reason (Seller will received this email):', '');
    ...
    this.adminService.rejectListing(id, reason.trim()).subscribe({
      ...
      error: () => this.error.set('Could not reject the listing at this time.'),
    });
  }
